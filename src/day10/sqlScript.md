...
- 별칭 인식 순서: 가운데 세 줄은 인식 안함. 위랑 똑같은 결과 출력.
...
**3) OUTER JOIN 조인**
- 한 쪽 테이블에는 해당하는 데이터가 존재하는데 다른 테이들에는 데이터가 존재하지 않을 때에도 모든 데이터를 추출하도록 하는 JOIN 방법(null 값도 포함해서 전체 데이터 출력)   
...
**<4> 상호 관련 서브쿼리 ***** (이중포문과 같음. 조건을 만족하면 select가 뜨고-로우가 하나 추가됨, 그 매칭이 끝날 때까지 도는 것)
...
ENGINE=InnoDB
;
(primary : null,중복 허용하지 않음. ID 값을 입력할 수 있는 키. 고유식별키(이름은 같아도 번호는 모두 다른))

insert into member (NO, NAME, address) VALUES (1,'김구리','노량진');   
...
```
ex)
-- 오토커밋 해제(원래 오토커밋 되어있는데 해제하면 나에겐 반영된 데이터가 보이지만 다른 이들은 보이지 않음. 
-> commit을 하면 저장됨 / rollback을 하면 다른 이들에게 보이는 것처럼 실제로 그 값이 저장된 것은 아니기에 보이지 않음 / commit 후 rollback을 하면 모두 반영되어 보임.
-> 오토커밋을 해제하는 이유는 시스템 상 건드려지게 되면 큰일나는 것들을 보호하기 위해서. 원데이터가 전부 손실되는 것을 막기 위해 수정한 부분을 통째로 원래대로 되돌리는 것임. 그런데 이 상태로 여러 명이 
작업할 경우 이 쿼리를 한 사람이 계속 잡고 있는 것이기 때문에 다른 이들은 접근이 불가해서 오류가 나는 경우도 있으니 주의!!
